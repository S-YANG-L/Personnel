...
function selectOrder() {
    ...
    if (state == "1") {
        document.getElementById("state").innerHTML = "待办详情";
    }
    if (state == "2") {
        document.getElementById("state").innerHTML = "已完成详情";
        document.getElementById("ACCOMPLISH_TIME1").style.display = "";
    }
    if (state == "3") {
        document.getElementById("state").innerHTML = "拒接详情";
    }
    $("#orderID").val(id);
    summer.ajax({
        type : 'post',
        ...
        if (state == "200") {
            ...
            //图片
            var batchno = orderBusinessDraft.processPicture;
            var batchnoImage = orderBusinessDraft.repairImage;
            //报修图片
            getprocessPicture(batchno);
            ...
            if (orderBusinessDraft.orderType == "0") {
                ...
                document.getElementById("chuliIm").style.display = "";
                //备注
...
//获取处理图片
function getprocessPicture(batchno) {
    summer.ajax({
        "header" : {
            //"Content-Type" : "application/json"
            Authorization : "OAuth2: token"
